...
public class ReplayMod implements Module, Scheduler {
   ...
   private static PathPackResources createJGuiResourcePack() {
      ...
      return new PathPackResources("replaymod_jgui", folder.toPath(), true) {
   ...
   public void registerKeyBindings(KeyBindingRegistry registry) {
      registry.registerKeyBinding("key.replaymod.settings", 0, () -> {
   ...
   private void printToChat(boolean warning, String message, Object... args) {
      if (this.getSettingsRegistry().get(Setting.NOTIFICATIONS)) {
         ...
         Component text = Component.literal("[").setStyle(coloredDarkGray).append(Component.translatable("replaymod.title").setStyle(coloredGold)).append(Component.literal("] ")).append(Component.translatable(message, args).setStyle(alert));
