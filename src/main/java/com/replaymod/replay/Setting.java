...
public final class Setting<T> extends SettingsRegistry.SettingKeys<T> {
   ...
   static {
      MAIN_MENU_BUTTON = new SettingsRegistry.MultipleChoiceSettingKeys("replay", "mainMenuButton", (String)null, GuiHandler.MainMenuButtonPosition.DEFAULT.name());
      MAIN_MENU_BUTTON.setChoices((List)Arrays.stream(GuiHandler.MainMenuButtonPosition.values()).map(Enum::name).collect(Collectors.toList()));
   }
}
