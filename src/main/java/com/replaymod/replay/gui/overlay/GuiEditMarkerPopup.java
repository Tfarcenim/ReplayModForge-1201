...
public class GuiEditMarkerPopup extends AbstractGuiPopup<GuiEditMarkerPopup> implements Typeable {
   ...
   public GuiEditMarkerPopup(GuiContainer container, Marker marker, Consumer<Marker> onSave) {
      ...
      this.buttons = (GuiPanel)((GuiPanel)(new GuiPanel()).setLayout((new HorizontalLayout(HorizontalLayout.Alignment.CENTER)).setSpacing(7))).addElements(new HorizontalLayout.Data(0.5D), new GuiElement[]{this.saveButton, this.cancelButton});
      this.onSave = onSave;
      this.setBackgroundColor(Colors.DARK_TRANSPARENT);
      ((GuiPanel)this.popup.setLayout((new VerticalLayout()).setSpacing(5))).addElements(new VerticalLayout.Data(0.5D), new GuiElement[]{this.title, this.inputs, this.buttons});
