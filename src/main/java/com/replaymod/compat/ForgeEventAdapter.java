...
public class ForgeEventAdapter extends EventRegistrations {
    ...
    public void onGuiRender(ScreenEvent.Render.Post event) {
        PostRenderScreenCallback.EVENT.invoker().postRenderScreen(event.getGuiGraphics(), getPartialTicks(event));
    }
    ...
    public void renderOverlay(CustomizeGuiOverlayEvent.DebugText event) {
        RenderHudCallback.EVENT.invoker().renderHud(event.getGuiGraphics(), getPartialTicks(event));
    }
