...
public class SPTimeline implements PathingRegistry {
   ...
   public Change setDefaultInterpolator(Interpolator interpolator) {
      ...
      Change change = CombinedChange.create(this.positionPath.getSegments().stream().filter((s) -> {
   ...
   public void addPositionKeyframe(long time, double posX, double posY, double posZ, float yaw, float pitch, float roll, int spectated) {
      ReplayModSimplePathing.LOGGER.debug("Adding position keyframe at {} pos {}/{}/{} rot {}/{}/{} entId {}", time, posX, posY, posZ, yaw, pitch, roll, spectated);
   ...
   public Change updatePositionKeyframe(long time, double posX, double posY, double posZ, float yaw, float pitch, float roll) {
      ReplayModSimplePathing.LOGGER.debug("Updating position keyframe at {} to pos {}/{}/{} rot {}/{}/{}", time, posX, posY, posZ, yaw, pitch, roll);
      Keyframe keyframe = this.positionPath.getKeyframe(time);
      ...
      Change change = UpdateKeyframeProperties.create(this.positionPath, keyframe).setValue(CameraProperties.POSITION, Triple.of(posX, posY, posZ)).setValue(CameraProperties.ROTATION, Triple.of(yaw, pitch, roll)).done();
      change.apply(this.timeline);
      return change;
   }

   public void removePositionKeyframe(long time) {
      ReplayModSimplePathing.LOGGER.debug("Removing position keyframe at {}", time);
      Path path = this.positionPath;
      Keyframe keyframe = path.getKeyframe(time);
   ...
   public void addTimeKeyframe(long time, int replayTime) {
      ReplayModSimplePathing.LOGGER.debug("Adding time keyframe at {} time {}", time, replayTime);
   ...
   public Change updateTimeKeyframe(long time, int replayTime) {
      ...
      Preconditions.checkState(keyframe != null, "Keyframe does not exists");
      Change change = UpdateKeyframeProperties.create(this.timePath, keyframe).setValue(TimestampProperty.PROPERTY, replayTime).done();
      change.apply(this.timeline);
      return change;
   }

   public void removeTimeKeyframe(long time) {
      ReplayModSimplePathing.LOGGER.debug("Removing time keyframe at {}", time);
      Path path = this.timePath;
      Keyframe keyframe = path.getKeyframe(time);
      Preconditions.checkState(keyframe != null, "No keyframe at that time");
      Change change = RemoveKeyframe.create(path, keyframe);
   ...
   public Change setInterpolatorToDefault(long time) {
      ...
      Keyframe keyframe = this.positionPath.getKeyframe(time);
      Preconditions.checkState(keyframe != null, "Keyframe does not exists");
      Change change = UpdateKeyframeProperties.create(this.positionPath, keyframe).removeProperty(ExplicitInterpolationProperty.PROPERTY).done();
      change.apply(this.timeline);
      return CombinedChange.createFromApplied(change, this.updateInterpolators());
   }

   public Change setInterpolator(long time, Interpolator interpolator) {
      ReplayModSimplePathing.LOGGER.debug("Setting interpolator of position keyframe at {} to {}", time, interpolator);
      Keyframe keyframe = this.positionPath.getKeyframe(time);
      Preconditions.checkState(keyframe != null, "Keyframe does not exists");
      PathSegment segment = this.positionPath.getSegments().stream().filter((s) -> {
         return s.getStartKeyframe() == keyframe;
      }).findFirst().orElseThrow(() -> {
         return new IllegalStateException("Keyframe has no following segment.");
      });
      this.registerPositionInterpolatorProperties(interpolator);
      Change change = CombinedChange.create(UpdateKeyframeProperties.create(this.positionPath, keyframe).setValue(ExplicitInterpolationProperty.PROPERTY, ObjectUtils.NULL).done(), SetInterpolator.create(segment, interpolator));
      change.apply(this.timeline);
      return CombinedChange.createFromApplied(change, this.updateInterpolators());
   }
   ...
   public Change moveKeyframe(SPTimeline.SPPath spPath, long oldTime, long newTime) {
      ...
      if (Iterables.getLast(path.getKeyframes()) != newKf) {
         ...
         restoreInterpolatorChange = path.getSegments().stream().filter((s) -> {
            return s.getEndKeyframe() == newKf;
         }).findFirst().flatMap((segment) -> {
            return lostInterpolator.map((interpolator) -> {
               return newKf.getValue(ExplicitInterpolationProperty.PROPERTY).isPresent()
      ...
      if (spPath == SPTimeline.SPPath.POSITION) {
         ...
         if (path.getSegments().size() == 1) {
            ...
            interpolatorUpdateChange = SetInterpolator.create(path.getSegments().iterator().next(),
                    firstInterpolator.get());
         } else {
      ...
      return CombinedChange.createFromApplied(removeChange, addChange, propertyChange, restoreInterpolatorChange,
              interpolatorUpdateChange, spectatorChange);
   }
   ...
   private Change updateInterpolators() {
      ...
      Iterator<PathSegment> var14 = pathSegments.iterator();

      while(true) {
         while(var14.hasNext()) {
            PathSegment segment = var14.next();
            if (this.isSpectatorSegment(segment)) {
               lastInterpolator = null;
         ...
         Change change = CombinedChange.create(updates.entrySet().stream().map((e) -> {
            return SetInterpolator.create(e.getKey(), e.getValue());
         }).toArray(Change[]::new));
         change.apply(this.timeline);
         return change;
      }
      ...
      if (this.entityTracker == null) {
         ...
         for (Keyframe keyframe : this.positionPath.getKeyframes()) {
            ...
            if (spectator.isPresent()) {
               ...
               if (time.isPresent()) {
                  Location expected = this.entityTracker.getEntityPositionAtTimestamp(spectator.get(), (long) time.get());
                  if (expected != null) {
                     Triple<Double, Double, Double> pos = keyframe.getValue(CameraProperties.POSITION).orElse(Triple.of(0.0D, 0.0D, 0.0D));
                     Triple<Float, Float, Float> rot = keyframe.getValue(CameraProperties.ROTATION).orElse(Triple.of(0.0F, 0.0F, 0.0F));
                     Location actual = new Location(pos.getLeft(), pos.getMiddle(), pos.getRight(), rot.getLeft(), rot.getRight());
                     if (!expected.equals(actual)) {
                        changes.add(UpdateKeyframeProperties.create(this.positionPath, keyframe).setValue(CameraProperties.POSITION, Triple.of(expected.getX(), expected.getY(), expected.getZ())).setValue(CameraProperties.ROTATION, Triple.of(expected.getYaw(), expected.getPitch(), 0.0F)).done());
                     }
                  }
               }
            }
