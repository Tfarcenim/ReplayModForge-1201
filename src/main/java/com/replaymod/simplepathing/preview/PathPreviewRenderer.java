...
public class PathPreviewRenderer extends EventRegistrations {
                    ...
                    if (timeline != null) {
                        ...
                        if (!path.getKeyframes().isEmpty()) {
                            ...
                            try {
                                ...
                                Iterator<PathSegment> var11 = path.getSegments().iterator();
                                ...
                                while (true) {
                                    ...
                                    do {
                                        if (!var11.hasNext()) {
                                            ...
                                            GL11.glDisable(2929);
                                            path.getKeyframes().stream().map(k -> Pair.of(k, k.getValue(CameraProperties.POSITION)
                                                            .map(this::tripleD2Vec))).filter(p -> p.getRight().isPresent()).map(p -> Pair.of(p.getLeft(), p.getRight().get()))
                                                    .filter(p -> distanceSquared(p.getRight(),
                                                            viewPos) < (double) renderDistanceSquared).sorted(new PathPreviewRenderer.KeyframeComparator(viewPos))
                                                    .forEachOrdered(p -> this.drawPoint(viewPos, p.getRight(),
                                                            p.getLeft()));
                                            ...
                                            if (entityId.isPresent()) {
                                                ...
                                                replayTime = path.getValue(CameraProperties.POSITION, time)
                                                        .map(this::tripleD2Vec);
                                                Optional<Vector3f> cameraRot = path
                                                        .getValue(CameraProperties.ROTATION, time)
                                        ...
                                        PathSegment segment = var11.next();
                                    ...
                                    for (int i = 0; (long) i <= steps; ++i) {
                                        ...
                                        if (spectator) {
                                            ...
                                            if (entityId.isPresent() && replayTime.isPresent()) {
                                                Location loc = entityTracker.getEntityPositionAtTimestamp(
                                                        (Integer) entityId.get(), (long) replayTime.get());
                                                if (loc != null) {
    ...
    private static int interpolateColor(int c1, int c2, double weight) {
        return interpolateColorComponent(c1 >> 16 & 255, c2 >> 16 & 255, weight) << 16 | interpolateColorComponent(c1 >> 8 & 255, c2 >> 8 & 255, weight) << 8 | interpolateColorComponent(c1 & 255, c2 & 255, weight);
    }
    ...
    private void drawPoint(Vector3f view, Vector3f pos, Keyframe keyframe) {
        ...
        RenderSystem.getModelViewStack().mulPose(MCVer.quaternion(-mc.getEntityRenderDispatcher().camera.getYRot(), new org.joml.Vector3f(0.0F, 1.0F, 0.0F)));
        RenderSystem.getModelViewStack().mulPose(MCVer.quaternion(mc.getEntityRenderDispatcher().camera.getXRot(), new org.joml.Vector3f(1.0F, 0.0F, 0.0F)));
        RenderSystem.applyModelViewMatrix();
    ...
    private void drawCamera(Vector3f view, Vector3f pos, Vector3f rot) {
        ...
        Vector3f t = Vector3f.sub(pos, view, null);
        RenderSystem.getModelViewStack().translate(t.x, t.y, t.z);
        RenderSystem.getModelViewStack().mulPose(MCVer.quaternion(-rot.x, new org.joml.Vector3f(0.0F, 1.0F, 0.0F)));
        RenderSystem.getModelViewStack().mulPose(MCVer.quaternion(rot.y, new org.joml.Vector3f(1.0F, 0.0F, 0.0F)));
        RenderSystem.getModelViewStack().mulPose(MCVer.quaternion(rot.z, new org.joml.Vector3f(0.0F, 0.0F, 1.0F)));
        Tesselator tessellator = Tesselator.getInstance();
        BufferBuilder buffer = tessellator.getBuilder();
        buffer.begin(Mode.LINES, DefaultVertexFormat.POSITION_COLOR_NORMAL);
        ...
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r).uv(0.5F, 0.125F).color(255, 255, 255, 200).endVertex();
        ...
        buffer.vertex(r + (double) cubeSize, r, r + (double) cubeSize).uv(0.25F, 0.25F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r + (double) cubeSize).uv(0.25F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r, r + (double) cubeSize, r + (double) cubeSize).uv(0.125F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r, r, r + (double) cubeSize).uv(0.125F, 0.25F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r).uv(0.0F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r + (double) cubeSize).uv(0.125F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r, r + (double) cubeSize).uv(0.125F, 0.25F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r, r).uv(0.0F, 0.25F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r, r + (double) cubeSize, r + (double) cubeSize).uv(0.25F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r, r + (double) cubeSize, r).uv(0.375F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r, r, r).uv(0.375F, 0.25F).color(255, 255, 255, 200).endVertex();
        ...
        buffer.vertex(r + (double) cubeSize, r, r + (double) cubeSize).uv(0.375F, 0.125F).color(255, 255, 255, 200).endVertex();
        ...
        buffer.vertex(r, r + (double) cubeSize, r + (double) cubeSize).uv(0.125F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r + (double) cubeSize).uv(0.25F, 0.125F).color(255, 255, 255, 200).endVertex();
        buffer.vertex(r + (double) cubeSize, r + (double) cubeSize, r).uv(0.25F, 0.0F).color(255, 255, 255, 200).endVertex();
        RenderSystem.applyModelViewMatrix();
