...
public final class Setting<T> extends SettingsRegistry.SettingKeys<T> {
   ...
   public static final Setting<Integer> TIMELINE_LENGTH = make("timelineLength", null, 1800);
   ...
   static {
      String format = "replaymod.gui.editkeyframe.interpolator.%s";
      DEFAULT_INTERPOLATION = new SettingsRegistry.MultipleChoiceSettingKeys<>("simplepathing", "interpolator", "replaymod.gui.settings.interpolator", String.format(format, InterpolatorType.fromString("invalid returns default").getLocalizationKey()));
      DEFAULT_INTERPOLATION.setChoices(Arrays.stream(InterpolatorType.values()).filter((i) -> {
