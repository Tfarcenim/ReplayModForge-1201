...
public class GuiPathing {
   ...
   public GuiPathing(ReplayMod core, ReplayModSimplePathing mod, ReplayHandler replayHandler) {
      ...
      this.renderButton = new GuiButton()
      ...
      this.positionKeyframeButton = new GuiButton() {
         public GuiElement getTooltip(RenderInfo renderInfo) {
            GuiTooltip tooltip = (GuiTooltip) super.getTooltip(renderInfo);
            if (tooltip != null) {
               String label;
      ...
      this.timeKeyframeButton = new GuiButton() {
         public GuiElement getTooltip(RenderInfo renderInfo) {
            GuiTooltip tooltip = (GuiTooltip) super.getTooltip(renderInfo);
            if (tooltip != null) {
               String label;
      ...
      this.timeline = new GuiKeyframeTimeline(this) {
         public void draw(GuiRenderer renderer, ReadableDimension size, RenderInfo renderInfo) {
            if (GuiPathing.this.player.isActive()) {
               this.setCursorPosition((int) GuiPathing.this.player.getTimePassed())
      ...
      this.scrollbar = new GuiHorizontalScrollbar().setSize(Integer.MAX_VALUE, 9);
      this.scrollbar.onValueChanged(new Runnable() {
         public void run() {
      ...
      this.zoomInButton = new GuiButton().setSize(9,
      ...
      this.zoomOutButton = new GuiButton()
      ...
      this.zoomButtonPanel = new GuiPanel()
              .setLayout(new VerticalLayout(VerticalLayout.Alignment.CENTER).setSpacing(2))
              .addElements(null, this.zoomInButton, this.zoomOutButton);
      this.timelinePanel = new GuiPanel().setSize(Integer.MAX_VALUE, 40)
              .setLayout(new CustomLayout<GuiPanel>() {
                 protected void layout(GuiPanel container, int width, int height) {
                    ...
                    this.size(GuiPathing.this.scrollbar, this.x(GuiPathing.this.zoomButtonPanel) - 2, 9);
                      ...
                      this.timeline, this.timelineTime, this.scrollbar, this.zoomButtonPanel);
      this.panel = new GuiPanel()
              .setLayout(new HorizontalLayout(HorizontalLayout.Alignment.CENTER).setSpacing(5))
              .addElements(new HorizontalLayout.Data(0.5D), this.playPauseButton,
      ...
      this.timeline.setLength(core.getSettingsRegistry().get(Setting.TIMELINE_LENGTH) * 1000);
      this.playPauseButton.setSpriteUV(new ReadablePoint() {
         ...
         public void run() {
            if (player.isActive()) {
               ...
               boolean ignoreTimeKeyframes = MCVer.Keyboard.isKeyDown(MCVer.Keyboard.KEY_LSHIFT);
               ...
               // beginning)
               int startTime = MCVer.Keyboard.isKeyDown(MCVer.Keyboard.KEY_LCONTROL) ? 0
      ...
      this.positionKeyframeButton.setSpriteUV(new ReadablePoint() {
         public int getX() {
         ...
         public int getY() {
            ...
            if (keyframePath != SPTimeline.SPPath.POSITION) {
               keyframePath = mod.getCurrentTimeline().isPositionKeyframe(
                       GuiPathing.this.timeline.getCursorPosition()) ? SPTimeline.SPPath.POSITION : null;
            }
      ...
      this.timeKeyframeButton.setSpriteUV(new ReadablePoint() {
         public int getX() {
            return 0;
         }
         ...
         public int getY() {
            ...
            if (keyframePath != SPTimeline.SPPath.TIME) {
               keyframePath = mod.getCurrentTimeline().isTimeKeyframe(
                       GuiPathing.this.timeline.getCursorPosition()) ? SPTimeline.SPPath.TIME : null;
            }
      ...
      this.overlay.addElements(null, this.panel);
   ...
   public void keyframeRepoButtonPressed() {
      ...
      try {
         TimelineSerialization serialization = new TimelineSerialization(this.mod.getCurrentTimeline(), null);
         String serialized = serialization.serialize(Collections.singletonMap("", this.mod.getCurrentTimeline().getTimeline()));
         Timeline currentTimeline = serialization.deserialize(serialized).get("");
         GuiKeyframeRepository gui = new GuiKeyframeRepository(this.mod.getCurrentTimeline(), this.replayHandler.getReplayFile(), currentTimeline);
   ...
   public void clearKeyframesButtonPressed() {
      GuiYesNoPopup.open(this.replayHandler.getOverlay(), new GuiLabel().setI18nText("replaymod.gui.clearcallback.title", new Object[0]).setColor(Colors.BLACK)).setYesI18nLabel("gui.yes").setNoI18nLabel("gui.no").onAccept(() -> {
   ...
   private Integer computeSyncTime(int cursor) {
      int time = this.replayHandler.getReplaySender().currentTimeStamp();
      Keyframe keyframe = this.mod.getCurrentTimeline().getTimePath().getKeyframes().stream().filter(it -> {
      ...
      if (updatedCursor != null) {
         ...
         while(true) {
            ...
            if (updatedCursor == null || updatedCursor == cursor) {
               this.timeline.setCursorPosition(cursor).ensureCursorVisibleWithPadding();
      ...
      new Thread(() -> {
         ...
         try {
            ...
            this.mod.getCore().runLater(() -> {
               this.mod.getCore().printWarningToChat("Error loading entity tracker: %s", var4.getLocalizedMessage());
   ...
   private Result<Timeline, String[]> preparePathsForPlayback(boolean ignoreTimeKeyframes) {
      ...
      if (errors != null) {
         ...
         try {
            ...
            Utils.error(ReplayModSimplePathing.LOGGER, this.replayHandler.getOverlay(), CrashReport.forThrowable(var7, "Cloning timeline"), () -> {
            });
   ...
   private String[] validatePathsForPlayback(SPTimeline timeline, boolean ignoreTimeKeyframes) {
      ...
      if (timeline.getPositionPath().getSegments().isEmpty()) {
         ...
         int time;
         for(Iterator<Keyframe> var4 = timeline.getTimePath().getKeyframes().iterator(); var4.hasNext(); lastTime = time) {
            Keyframe keyframe = var4.next();
            time = keyframe.getValue(TimestampProperty.PROPERTY).orElseThrow(IllegalStateException::new);
            if (time < lastTime) {
   ...
   public boolean loadEntityTracker(Runnable thenRun) {
      if (this.entityTracker == null && !this.errorShown) {
         ...
         final GuiPathing.LoadEntityTrackerPopup popup = new GuiPathing.LoadEntityTrackerPopup(this.replayHandler.getOverlay());
   ...
   public void toggleKeyframe(SPTimeline.SPPath path, boolean neverSpectator) {
      ...
      if (this.loadEntityTracker(() -> {
         ...
         if (timeline.getPositionPath().getKeyframes().isEmpty() && timeline.getTimePath().getKeyframes().isEmpty() && time > 1000) {
            String text = I18n.get("replaymod.gui.ingame.first_keyframe_not_at_start_warning");
            GuiInfoPopup.open(this.overlay, text.split("\\\\n"));
         }
         ...
         case TIME:
            if (this.mod.getSelectedPath() == path) {
               ...
               this.mod.setSelected(path, time);
            }
            break;
         case POSITION:
            if (this.mod.getSelectedPath() == path) {
               ...
               timeline.addPositionKeyframe(time, camera.getX(), camera.getY(), camera.getZ(), camera.getYRot(), camera.getXRot(), camera.roll, spectatedId);
               this.mod.setSelected(path, time);
            }
         }
   ...
   private class LoadEntityTrackerPopup extends AbstractGuiPopup<GuiPathing.LoadEntityTrackerPopup> {
      ...
      public LoadEntityTrackerPopup(GuiContainer container) {
         super(container);
         this.progressBar = new GuiProgressBar(this.popup).setSize(300, 20).setI18nLabel("replaymod.gui.loadentitytracker");
         this.open();
      }
