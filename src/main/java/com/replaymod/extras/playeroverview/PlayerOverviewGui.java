...
public class PlayerOverviewGui extends GuiScreen implements Closeable {
   ...
   public PlayerOverviewGui(PlayerOverview extra, List<Player> players) {
      ...
      this.spectateLabel = (GuiLabel)(new GuiLabel(this.contentPanel)).setI18nText("replaymod.gui.playeroverview.spectate", new Object[0]);
      this.visibleLabel = (GuiLabel)(new GuiLabel(this.contentPanel)).setI18nText("replaymod.gui.playeroverview.visible", new Object[0]);
      this.playersScrollable = (GuiVerticalList)((GuiVerticalList)(new GuiVerticalList(this.contentPanel)).setDrawSlider(true)).setDrawShadow(true);
      this.saveCheckbox = (GuiCheckbox)((GuiCheckbox)(new GuiCheckbox(this.contentPanel)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.playeroverview.remembersettings.description", new Object[0]))).setI18nLabel("replaymod.gui.playeroverview.remembersettings", new Object[0]);
      ...
      }).setLabel("")).setChecked(true)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.playeroverview.showall", new Object[0]));
      ...
      }).setLabel("")).setChecked(false)).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.playeroverview.hideall", new Object[0]));
      this.setBackground(AbstractGuiScreen.Background.NONE);
      this.setTitle((GuiLabel)(new GuiLabel()).setI18nText("key.replaymod.playeroverview", new Object[0]));
      ...
      while(var3.hasNext()) {
         ...
         if (p instanceof AbstractClientPlayer) {
            ...
            final GuiClickable panel = (GuiClickable)((GuiClickable)((GuiClickable)(new GuiClickable()).setLayout((new HorizontalLayout()).setSpacing(2))).addElements(new HorizontalLayout.Data(0.5D), new GuiElement[]{(new GuiImage() {
            ...
            }).setSize(16, 16), ((GuiLabel)(new GuiLabel()).setText(p.getName().getString())).setColor(isSpectator(p) ? Colors.DKGREY : Colors.WHITE)})).onClick(new Runnable() {
               public void run() {
            ...
            ((GuiPanel)(new GuiPanel(this.playersScrollable.getListPanel())).setLayout(new CustomLayout<GuiPanel>() {
               protected void layout(GuiPanel container, int width, int height) {
                  this.pos(panel, 5, 0);
   ...
   private static final class PlayerComparator implements Comparator<Player> {
      public int compare(Player o1, Player o2) {
         if (PlayerOverviewGui.isSpectator(o1) && !PlayerOverviewGui.isSpectator(o2)) {
            ...
            return PlayerOverviewGui.isSpectator(o2) && !PlayerOverviewGui.isSpectator(o1) ? -1 : o1.getName().getString().compareToIgnoreCase(o2.getName().getString());
         }
      }
   }
}
