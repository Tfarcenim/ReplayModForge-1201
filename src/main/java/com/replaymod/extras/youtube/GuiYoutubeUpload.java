...
public class GuiYoutubeUpload extends GuiScreen {
      ...
      public void run() {
         ...
         if (GuiYoutubeUpload.this.upload == null) {
            if (problem == null) {
               ...
               GuiYoutubeUpload.this.uploadButton.setTooltip(null);
            } else {
               GuiYoutubeUpload.this.uploadButton.setDisabled();
               GuiYoutubeUpload.this.uploadButton.setTooltip((new GuiTooltip()).setI18nText(problem));
            }
         }

      }
   };
   public final GuiTextField nameField = (new GuiTextField()).setI18nHint("replaymod.gui.videotitle", new Object[0]).onTextChanged((s) -> {
   ...
   public GuiYoutubeUpload(GuiScreen previousScreen, File videoFile, int videoFrames, RenderSettings settings) {
      this.descriptionField.setText(new String[]{I18n.get("replaymod.gui.videodescription")});
      this.tagsField = (new GuiTextField()).setI18nHint("replaymod.gui.videotags");
      this.nameField.setNext(this.descriptionField).getNext().setNext(this.tagsField).getNext().setNext(this.nameField);
      this.progressBar = new GuiProgressBar();
      this.leftPanel = (new GuiPanel(this)).setLayout(new CustomLayout<GuiPanel>() {
      ...
      }).addElements(null, this.nameField, this.descriptionField, this.tagsField, this.progressBar);
      this.visibilityDropdown = new GuiDropdownMenu<VideoVisibility>().setSize(200, 20).setValues(VideoVisibility.values()).setSelected(VideoVisibility.PUBLIC);
      this.thumbnailButton = (new GuiButton()).onClick(new Runnable() {
         public void run() {
            GuiFileChooserPopup.openLoadGui(GuiYoutubeUpload.this, "replaymod.gui.load", ImageIO.getReaderFileSuffixes()).onAccept((file) -> {
               ...
               try {
                  ...
                  ImageInputStream in = ImageIO.createImageInputStream(new ByteArrayInputStream(GuiYoutubeUpload.this.thumbnailImage));
      ...
      this.thumbnail = (new GuiImage()).setSize(200, 112).setTexture(Utils.DEFAULT_THUMBNAIL);
      this.uploadButton = (new GuiButton(this)).setSize(98, 20);
      this.closeButton = (new GuiButton(this)).onClick(new Runnable() {
         public void run() {
            GuiYoutubeUpload.this.previousScreen.display();
         }
      }).setSize(98, 20).setI18nLabel("replaymod.gui.back");
      this.rightPanel = (new GuiPanel(this)).addElements(null, new GuiElement[]{this.visibilityDropdown, this.thumbnailButton, this.thumbnail}).setLayout((new VerticalLayout(VerticalLayout.Alignment.TOP)).setSpacing(10));
   ...
   private void setState(boolean uploading) {
      ...
      if (uploading) {
         ...
         this.uploadButton.onClick(() -> {
            try {
               this.setState(true);
               VideoVisibility visibility = this.visibilityDropdown.getSelectedValue();
               ...
               this.upload = new YoutubeUploader(this.getMinecraft(), this.videoFile, this.videoFrames, this.thumbnailFormat, this.thumbnailImage, this.settings, visibility, snippet);
               ...
               Futures.addCallback(future, new FutureCallback<Video>() {
                  public void onSuccess(Video result) {
                     ...
                     GuiYoutubeUpload.this.progressBar.setLabel(I18n.get("replaymod.gui.ytuploadprogress.done", url));
   ...
   public void draw(GuiRenderer renderer, ReadableDimension size, RenderInfo renderInfo) {
      if (this.upload != null && this.upload.getState() != null) {
         ...
         this.progressBar.setI18nLabel("replaymod.gui.ytuploadprogress." + this.upload.getState().name().toLowerCase());
      }
