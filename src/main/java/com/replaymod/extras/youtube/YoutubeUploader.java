...
public class YoutubeUploader {
   ...
   public ListenableFuture<Video> upload() throws IOException {
      ...
      this.thread = new Thread(() -> {
         try {
            ...
            YouTube youTube = (new YouTube.Builder(this.httpTransport, JSON_FACTORY, credential)).setApplicationName("ReplayMod").build();
   ...
   private Credential auth() throws IOException {
      GoogleAuthorizationCodeFlow flow = (new GoogleAuthorizationCodeFlow.Builder(this.httpTransport, JSON_FACTORY, "743126594724-mfe7pj1k7e47uu5pk4503c8st9vj9ibu.apps.googleusercontent.com", "gMwcy3mRYCRamCIjJIYP7rqc", Collections.singleton("https://www.googleapis.com/auth/youtube.upload"))).setDataStoreFactory(this.dataStoreFactory).build();
   ...
   private File preUpload() throws InterruptedException, IOException {
      ...
      if (this.settings.getRenderMethod().isSpherical()) {
         ...
         if (!isMp4) {
            ...
            String args = String.format("-i %s -c:v libx264 -preset slow -crf 16 %s", this.videoFile.getName(), outputFile.getName());
            CommandLine commandLine = new CommandLine(this.settings.getExportCommandOrDefault());
            commandLine.addArguments(args);
            ReplayModExtras.LOGGER.info("Re-encoding for metadata injection with {} {}", commandLine.getExecutable(), args);
   ...
   private void doThumbUpload(YouTube youTube, Video video) throws IOException {
      try {
         ...
         GoogleJsonError.ErrorInfo info = var8.getDetails().getErrors().get(0);
