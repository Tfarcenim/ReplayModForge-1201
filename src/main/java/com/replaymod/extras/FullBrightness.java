...
public class FullBrightness extends EventRegistrations implements Extra {
   ...
   public void register(ReplayMod mod) throws Exception {
      ...
      mod.getKeyBindingRegistry().registerKeyBinding("key.replaymod.lighting", 90, new Runnable() {
         public void run() {
   ...
   private void preRender() {
      if (this.active && this.module.getReplayHandler() != null) {
         ...
         if (type == FullBrightness.Type.Gamma || type == FullBrightness.Type.Both) {
            this.originalGamma = this.mc.options.gamma().get();
            ((SimpleOptionAccessor)(Object)this.mc.options.gamma()).setRawValue(1000.0D);
         }

         if ((type == FullBrightness.Type.NightVision || type == FullBrightness.Type.Both) && this.mc.player != null) {
   ...
   private void postRender() {
      if (this.active && this.module.getReplayHandler() != null) {
         FullBrightness.Type type = this.getType();
         if (type == FullBrightness.Type.Gamma || type == FullBrightness.Type.Both) {
            ((SimpleOptionAccessor)(Object)this.mc.options.gamma()).setRawValue(this.originalGamma);
         }
   ...
   private void updateIndicator(GuiReplayOverlay overlay) {
      if (this.active) {
         overlay.statusIndicatorPanel.addElements(new HorizontalLayout.Data(1.0D), this.indicator);
      } else {
