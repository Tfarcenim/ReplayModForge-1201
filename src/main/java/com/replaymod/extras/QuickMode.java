...
public class QuickMode extends EventRegistrations implements Extra {
   ...
   public QuickMode() {
      this.indicator = (GuiImage)((GuiImage)(new GuiImage()).setTexture(ReplayMod.TEXTURE, 40, 100, 16, 16)).setSize(16, 16);
   ...
   public void register(ReplayMod mod) {
      ...
      mod.getKeyBindingRegistry().registerKeyBinding("key.replaymod.quickmode", 81, () -> {
   ...
   private void updateIndicator(GuiReplayOverlay overlay, boolean enabled) {
      if (enabled) {
         overlay.statusIndicatorPanel.addElements(new HorizontalLayout.Data(1.0D), new GuiElement[]{this.indicator});
      } else {
