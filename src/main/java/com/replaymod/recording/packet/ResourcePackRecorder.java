...
public class ResourcePackRecorder {
   ...
   public synchronized ClientboundResourcePackPacket handleResourcePack(Connection netManager, ClientboundResourcePackPacket packet) {
      ...
      if (url.startsWith("level://")) {
         ...
         if (serverData != null && serverData.getResourcePackStatus() == ServerPackStatus.ENABLED) {
            netManager.send(this.makeStatusPacket(hash, Action.ACCEPTED));
            this.downloadResourcePackFuture(netManager, requestId, url, hash);
         } else if (serverData != null && serverData.getResourcePackStatus() != ServerPackStatus.PROMPT) {
            netManager.send(this.makeStatusPacket(hash, Action.DECLINED));
         } else {
            mc.execute(() -> {
               mc.setScreen(new ConfirmScreen((result) -> {
                  if (serverData != null) {
                     serverData.setResourcePackStatus(result ? ServerPackStatus.ENABLED : ServerPackStatus.DISABLED);
                  }
   ...
   public interface IDownloadingPackFinder {
      void setRequestCallback(Consumer<File> var1);
   }
}
