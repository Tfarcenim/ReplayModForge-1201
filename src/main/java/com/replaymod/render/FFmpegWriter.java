...
public class FFmpegWriter implements FrameConsumer<BitmapFrame> {
   ...
   private void checkSize(int width, int height) {
      Validate.isTrue(width == this.settings.getVideoWidth(), "Width has to be %d but was %d", this.settings.getVideoWidth(), width);
      Validate.isTrue(height == this.settings.getVideoHeight(), "Height has to be %d but was %d", this.settings.getVideoHeight(), height);
   }
