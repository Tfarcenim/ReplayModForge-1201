...
public class DMesh {
      ...
      return serializer.maybeMajor(this, this.id, Mesh.class, () -> {
         ...
         for (DMaterial material : this.materials) {
            ...
            images.add(material.textures.isEmpty() ? null
                    : material.textures.get(0).texture.image.serialize(serializer));
         }
         ...
         return (mesh) -> {
            ...
            if (!this.vertices.isEmpty()) {
               mPolys = serializer.writeData(MVert.class, this.vertices.size(), (ixxx, mVert) -> {
                  this.vertices.get(ixxx).serialize(mVert);
               });
               eDataLayer = serializer.writeData(CustomDataLayer.class);
            ...
            if (!this.edges.isEmpty()) {
               mPolys = serializer.writeData(MEdge.class, this.edges.size(), (ixxx, mEdge) -> {
                  this.edges.get(ixxx).serialize(mEdge);
               });
               eDataLayer = serializer.writeData(CustomDataLayer.class);
               ...
               mesh.setMface(serializer.writeData(MFace.class, this.faces.size(), (ixxx, mFace) -> {
                  this.faces.get(ixxx).serialize(mFace);
               }));
            ...
            if (!this.polys.isEmpty()) {
               mPolys = serializer.writeData(MPoly.class, this.polys.size(), (ixxx, mPoly) -> {
                  this.polys.get(ixxx).serialize(mPoly);
               });
               mTexPolys = serializer.writeData(MTexPoly.class, this.polys.size(), (ixxx, mTexPoly) -> {
                  mTexPoly.setTpage(images.get(this.polys.get(ixxx).materialSlot));
               });
   ...
   public void addTriangle(DMesh.Vertex v1, DMesh.Vertex v2, DMesh.Vertex v3, Vector2f uv1, Vector2f uv2, Vector2f uv3, int c1, int c2, int c3, int materialSlot) {
   ...
   public void addQuad(DMesh.Vertex v1, DMesh.Vertex v2, DMesh.Vertex v3, DMesh.Vertex v4, Vector2f uv1, Vector2f uv2, Vector2f uv3, Vector2f uv4, int c1, int c2, int c3, int c4, int materialSlot) {
      int vOffset = this.vertices.size();
      int eOffset = this.edges.size();
      int lOffset = this.loops.size();
      this.vertices.add(v1);
   ...
   public boolean hasZeroLengthEdge(float delta) {
      ...
      do {
         ...
         v1 = this.vertices.get(edge.v1);
         v2 = this.vertices.get(edge.v2);
