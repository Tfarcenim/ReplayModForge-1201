...
public abstract class AbstractGuiContainer<T extends AbstractGuiContainer<T>> extends AbstractComposedGuiElement<T> implements GuiContainer<T> {
   ...
   public void layout(ReadableDimension size, RenderInfo renderInfo) {
      ...
      if (size != null) {
         ...
         Iterator<Entry<GuiElement, Pair<ReadablePoint, ReadableDimension>>> var3 = this.layedOutElements.entrySet().iterator();

         while(true) {
            Entry<GuiElement, Pair<ReadablePoint, ReadableDimension>> e;
            GuiElement element;
            while(true) {
               if (!var3.hasNext()) {
                  return;
   ...
   public T sortElements() {
      this.sortElements((o1, o2) -> o1 instanceof Comparable && o2 instanceof Comparable ? ((Comparable)o1).compareTo(o2) : o1.hashCode() - o2.hashCode());
      return this.getThis();
   }

   public T sortElements(Comparator<GuiElement> comparator) {
      this.elements = this.elements.entrySet().stream().sorted(Entry.comparingByKey(comparator)).collect(Collectors.toMap(Entry::getKey, Entry::getValue, (x, y) -> {
         return y;
      }, LinkedHashMap::new));
      return this.getThis();
   }
