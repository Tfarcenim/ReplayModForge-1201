...
public class Utils {
   ...
   public static void link(Focusable... focusables) {
      Preconditions.checkArgument((new HashSet(Arrays.asList(focusables))).size() == focusables.length, "focusables must be unique and not null");
   ...
   public static void drawDynamicRect(GuiRenderer renderer, int width, int height, int u, int v, int uWidth, int vHeight, int topBorder, int bottomBorder, int leftBorder, int rightBorder) {
      ...
      for(x = 0; x < 2; ++x) {
         ...
         renderer.drawTexturedRect(segmentWidth, height - bottomBorder, textureX, v + vHeight - bottomBorder, leftBorder, bottomBorder);
      }
