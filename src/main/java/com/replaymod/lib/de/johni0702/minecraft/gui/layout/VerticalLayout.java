...
public class VerticalLayout implements Layout {
   ...
   public Map<GuiElement, Pair<ReadablePoint, ReadableDimension>> layOut(GuiContainer<?> container, ReadableDimension size) {
      ...
      for(Iterator<Entry<GuiElement, LayoutData>> var6 = container.getElements().entrySet().iterator(); var6.hasNext(); y += elementSize.getHeight()) {
         Entry<GuiElement, LayoutData> entry = var6.next();
         y += spacing;
         spacing = this.spacing;
         GuiElement element = entry.getKey();
         VerticalLayout.Data data = entry.getValue() instanceof VerticalLayout.Data ? (VerticalLayout.Data)entry.getValue() : DEFAULT_DATA;
      ...
      if (this.alignment != VerticalLayout.Alignment.TOP) {
         ...
         for (Pair<ReadablePoint, ReadableDimension> readablePointReadableDimensionPair : map.values()) {
            ((Point) readablePointReadableDimensionPair.getLeft()).translate(0, remaining);
         }
      }
   ...
   public ReadableDimension calcMinSize(GuiContainer<?> container) {
      ...
      for(Iterator var5 = container.getElements().entrySet().iterator(); var5.hasNext(); height += minSize.getHeight()) {
