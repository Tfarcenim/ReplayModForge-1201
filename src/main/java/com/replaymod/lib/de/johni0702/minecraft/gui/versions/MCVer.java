...
public class MCVer {
   ...
   public static void drawRect(int right, int bottom, int left, int top) {
      ...
      vertexBuffer.begin(Mode.QUADS, DefaultVertexFormat.POSITION);
      vertexBuffer.vertex(right, top, 0.0D).endVertex();
      vertexBuffer.vertex(left, top, 0.0D).endVertex();
      vertexBuffer.vertex(left, bottom, 0.0D).endVertex();
      vertexBuffer.vertex(right, bottom, 0.0D).endVertex();
      tessellator.end();
   }
   ...
   public static void drawRect(int x, int y, int width, int height, ReadableColor tl, ReadableColor tr, ReadableColor bl, ReadableColor br) {
      ...
      vertexBuffer.vertex(x, y + height, 0.0D).color(bl.getRed(), bl.getGreen(), bl.getBlue(), bl.getAlpha()).endVertex();
      vertexBuffer.vertex(x + width, y + height, 0.0D).color(br.getRed(), br.getGreen(), br.getBlue(), br.getAlpha()).endVertex();
      vertexBuffer.vertex(x + width, y, 0.0D).color(tr.getRed(), tr.getGreen(), tr.getBlue(), tr.getAlpha()).endVertex();
      vertexBuffer.vertex(x, y, 0.0D).color(tl.getRed(), tl.getGreen(), tl.getBlue(), tl.getAlpha()).endVertex();
      tessellator.end();
   }
   ...
   private static class ScissorBounds {
      private static final MCVer.ScissorBounds DISABLED = new MCVer.ScissorBounds(0, 0, Integer.MAX_VALUE, Integer.MAX_VALUE);
      private final int x;
      private final int y;
