...
public class GuiEditReplay extends AbstractGuiPopup<GuiEditReplay> {
   ...
   protected GuiEditReplay(GuiContainer container, Path inputPath) throws IOException {
      super(container);
      this.zoomInButton = (new GuiButton()).setSize(9, 9).onClick(() -> {
         this.zoomTimeline(0.6666666666666666D);
      }).setTexture(ReplayMod.TEXTURE, 256).setSpriteUV(40, 20).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.ingame.menu.zoomin"));
      this.zoomOutButton = (new GuiButton()).setSize(9, 9).onClick(() -> {
         this.zoomTimeline(1.5D);
      }).setTexture(ReplayMod.TEXTURE, 256).setSpriteUV(40, 30).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.ingame.menu.zoomout"));
      this.zoomButtonPanel = (new GuiPanel()).setLayout((new VerticalLayout(VerticalLayout.Alignment.CENTER)).setSpacing(2)).addElements(null, this.zoomInButton, this.zoomOutButton);
      ...
      try {
         ...
         GuiMarkerTimeline var10000 = this.timeline.setSize(300, 20).setMarkers().setLength(replayFile.getMetaData().getDuration());
      ...
      GuiPanel timelinePanel = (new GuiPanel()).setSize(300, 40).setLayout(new CustomLayout<GuiPanel>() {
      ...
      }).addElements(null, this.timelineTime, this.timeline, this.scrollbar, this.zoomButtonPanel);
      GuiButton buttonAddSplit = (new GuiButton()).setSize(100, 20).setI18nLabel("replaymod.gui.edit.split", new Object[0]).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.edit.split.tooltip")).onClick(() -> {
         Marker marker = new Marker();
         marker.setTime(this.timeline.getCursorPosition());
         marker.setName("_RM_SPLIT");
         this.timeline.addMarker(marker);
      });
      GuiButton buttonInsertCut = (new GuiButton()).setSize(100, 20).setI18nLabel("replaymod.gui.edit.cut.start", new Object[0]).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.edit.cut.tooltip")).onClick(() -> {
         Marker marker = new Marker();
         marker.setTime(this.timeline.getCursorPosition());
         marker.setName("_RM_START_CUT");
         this.timeline.addMarker(marker);
      });
      GuiButton buttonEndCut = (new GuiButton()).setSize(100, 20).setI18nLabel("replaymod.gui.edit.cut.end", new Object[0]).setTooltip((new GuiTooltip()).setI18nText("replaymod.gui.edit.cut.tooltip")).onClick(() -> {
         Marker marker = new Marker();
         marker.setTime(this.timeline.getCursorPosition());
         marker.setName("_RM_END_CUT");
         this.timeline.addMarker(marker);
      });
      GuiPanel controlPanel = (new GuiPanel()).setLayout((new HorizontalLayout()).setSpacing(4)).addElements(null, buttonAddSplit, buttonInsertCut, buttonEndCut);
      GuiButton applyButton = (new GuiButton()).setI18nLabel("replaymod.gui.edit.apply", new Object[0]).setSize(150, 20).onClick(this::apply);
      GuiButton closeButton = (new GuiButton()).setI18nLabel("replaymod.gui.close", new Object[0]).setSize(150, 20).onClick(this::close);
      GuiPanel buttonPanel = (new GuiPanel()).setLayout((new HorizontalLayout()).setSpacing(8)).addElements(null, applyButton, closeButton);
      this.popup.setLayout((new VerticalLayout(VerticalLayout.Alignment.TOP)).setSpacing(10));
      this.popup.addElements(new VerticalLayout.Data(0.5D), timelinePanel, controlPanel, buttonPanel);
   }
   ...
   private void apply() {
      ...
      (new Thread(() -> {
         try {
            ...
            Utils.error(ReplayModEditor.LOGGER, this, CrashReport.forThrowable(var8, "Writing markers"), this::close);
         }

         try {
            GuiProgressBar var10001 = progressPopup.progressBar;
            Objects.requireNonNull(var10001);
            MarkerProcessor.apply(this.inputPath, var10001::setProgress);
   ...
   private class EditTimeline extends GuiMarkerTimeline {
      ...
      private void drawCutQuads(GuiRenderer renderer, ReadableDimension size) {
         boolean inCut = false;
         int startTime = 0;
         Iterator<Marker> var5 = this.markers.stream().sorted(Comparator.comparing(Marker::getTime)).collect(Collectors.toList()).iterator();
   ...
   private class ProgressPopup extends AbstractGuiPopup<GuiEditReplay.ProgressPopup> {
      ...
      ProgressPopup(GuiContainer container) {
         super(container);
         this.progressBar = (new GuiProgressBar(this.popup)).setSize(300, 20);
         this.open();
      }
