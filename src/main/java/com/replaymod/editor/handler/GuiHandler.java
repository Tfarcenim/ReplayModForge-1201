   ...
   public void injectIntoReplayViewer(Screen vanillaGuiScreen) {
      ...
      if (guiScreen instanceof GuiReplayViewer) {
         ...
         if (replayViewer.editorButton.getChildren().isEmpty()) {
            replayViewer.replaySpecificButtons.add((GuiButton)((GuiButton)((GuiButton)((GuiButton)(new GuiButton(replayViewer.editorButton)).onClick(() -> {
               if (!Utils.ifMinimalModeDoPopup(replayViewer, () -> {
               })) {
                  try {
                     (new GuiEditReplay(replayViewer, ((GuiReplayViewer.GuiReplayEntry)replayViewer.list.getSelected().get(0)).file.toPath()) {
                     ...
                     Utils.error(ReplayModEditor.LOGGER, replayViewer, CrashReport.forThrowable(var3, "Opening replay editor"), () -> {
                     });
                  }
